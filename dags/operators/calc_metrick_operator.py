...
class CalculateMetricsOperator(BaseOperator):
		...
		def execute(self, context):
				# Получаем данные из предыдущей задачи (FetchStockDataOperator) через XCom
				ti = context['ti']
		...
		def _add_market_metrics(self, df):
				...
				# Функция для расчёта RSI (индекс относительной силы) за 14 дней
		...
		def _calc_portfolio_metrics(self, df):
				...
				# Оставляем только те тикеры, для которых есть веса и данные
				...
				# Доходность портфеля в каждый день – взвешенная сумма доходностей активов
				...
				# Исторический VaR 95%: 5-й процентиль доходности за окно 252 дня (минимум 20 наблюдений)
